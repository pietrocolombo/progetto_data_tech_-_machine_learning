...
for(i_row in 1:nrow(dati))
{
  ...
  dati$state[i_row] <- as.character(info$state)
  dati$country[i_row] <- as.character(info$country)
  dati$city[i_row] <- as.character(info$city)
  dati$type_road[i_row] <- as.character(info$type_road)
